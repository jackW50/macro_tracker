class FoodComposition < ApplicationRecord
  ...
  scope :total_grams_of_macro, ->(food, macronutrient) { where('food_compositions.food_id = ? AND food_compositions.macronutrient_id = ?', food.id, macronutrient.id).pluck(:macronutrient_grams).first }
